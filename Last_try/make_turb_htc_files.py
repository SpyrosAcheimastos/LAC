...
def main(turbine_type='OUR_DESIGN'):
    ...
    cwd = Path.cwd()
    wsps = range(5, 23)  # wind speed range
    ...
    if turbine_type == 'DTU_10MW':
        ...
        master_htc = cwd / '_master/Jim_Design.htc'
        opt_path = cwd / 'data/Jim_Design_flex_blade_with_shaving.opt'
        htc_dir = cwd / 'htc_turb/'  # top-level folder to save htc files (can be path to gbar!)
        res_dir = cwd / 'res_turb/'  # where HAWC2 should save res files, relative to its working directory
