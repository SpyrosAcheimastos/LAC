...
def main(turbine_type='OUR_DESIGN'):
    ...
    if turbine_type == 'DTU_10MW':
        ...
        master_htc = cwd.parent / '_master/Jim_Design.htc'
        opt_path = cwd.parent / 'data/Jim_Design_flex_blade_with_shaving.opt'
        htc_dir = cwd.parent / 'htc_turb/'  # top-level folder to save htc files (can be path to gbar!)
        res_dir = cwd.parent / 'res_turb/'  # where HAWC2 should save res files, relative to its working directory
