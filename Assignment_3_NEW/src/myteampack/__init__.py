...
class MyHTC(HTCFile):
    ...
    def make_step(self, save_dir, append, ctrl_type, wsp_start, wsp_stop,
        ...
        # SPYROS: controller parameters
        min_rotor_speed = 6 * 2*np.pi/60                              # [rad/s]
        rated_rotor_speed = calc_omega_rated(tsr=7.73, cp=0.48127)    # [rad/s]
        minimum_pitch = 102     # SPYROS: In order to read my "wpdata.102"

        # Constant Parameters
        self.dll.type2_dll.init.constant__2 = 2, min_rotor_speed
        self.dll.type2_dll.init.constant__3 = 3, rated_rotor_speed
        self.dll.type2_dll.init.constant__4 = 4, maximum_allowable_rotor_torque # [Nm]
